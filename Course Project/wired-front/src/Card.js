...
function Card({ postOfCard, setPostsList }) {
  ...
  function deletePost() {
    setPostsList((prevState) => {
      return prevState.filter((post) => post.id != postOfCard.id);
    });
  }

  return (
    <div className="card" id={postOfCard.id}>
      <img src={postOfCard.imgUrl} alt="" />
      <span>{postOfCard.category}</span>
      <br />
      <span>
        <a href={"/posts.html?id=" + postOfCard.id}>{postOfCard.title}</a>
      </span>
      <p>{postOfCard.desc}</p>
      <span>{postOfCard.author.name + " " + postOfCard.author.sirname}</span>
